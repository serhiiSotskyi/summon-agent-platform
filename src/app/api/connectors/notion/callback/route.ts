...
export async function GET(request: NextRequest) {
  ...
  if (!code) {
    return NextResponse.json(
      { error: "Notion OAuth callback did not include an authorization code." },
      { status: 400 },
    );
  }
  ...
  try {
    const credential = await exchangeNotionOAuthCode({
      code,
      state,
    ...
    console.error("[connectors.notion.callback]", {
